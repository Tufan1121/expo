...
class solicitudes
{
    ...
    {
        $consulta = "SELECT pedido.total_pagar,pedido.pedidos,pedido.fecha,clientes.nom_cliente,clientes.ap_cliente FROM pedido INNER JOIN clientes ON pedido.idcliente=clientes.id_cliente where pedido.idexpo=".$expo." and pedido.estatus=1 ORDER BY pedido.fecha ASC";
        try {
            // Preparar sentencia
            $comando = Database::getInstance()->getDb()->prepare($consulta);
            // Ejecutar sentencia preparada
    ...
    {
        $consulta = "SELECT pedido.total_pagar,pedido.pedidos,pedido.fecha,clientes.nom_cliente,clientes.ap_cliente FROM pedido INNER JOIN clientes ON clientes.id_cliente=pedido.idcliente where pedido.estatus !=1 and pedido.estatus!=9 and pedido.idexpo=".$expo." ORDER BY pedido.fecha ASC";
        try {
            // Preparar sentencia
            $comando = Database::getInstance()->getDb()->prepare($consulta);
            // Ejecutar sentencia preparada
...
public static function getAlltodoinventarioupdate()
    ...
    {
        $consulta = "SELECT fecha FROM 	pedido where idexpo=".$idexpo." GROUP BY fecha ";
        try {
            // Preparar sentencia
            $comando = Database::getInstance()->getDb()->prepare($consulta);
            // Ejecutar sentencia preparada
    ...
    {
        // Consulta de la tabla Alumnos
        $consulta = "SELECT ".$tabla.".diseno,".$tabla.".producto1,".$tabla.".descripcio,".$tabla.".medidas,".$tabla.".hm, gproducto.pathima1,gproducto.precio1,gproducto.precio2, gproducto.precio3 FROM ".$tabla." INNER JOIN gproducto WHERE ".$tabla.".producto=gproducto.producto AND ".$tabla.".descripcio LIKE ?";
