...
//echo "observaciones " . $actor['observaciones'] . " " . $actor['pedidos'] . " .";
	...
	$estatux=$tablapedido['estatus'];
	$usuario=$tablapedido['idusuario'];
	
//Busca Usuario
$sql = "SELECT * FROM user WHERE idusuario = '$usuario'";
if (!$resultadu = $mysqli->query($sql)) {
    echo "Lo sentimos, este sitio web está experimentando problemas.";
    echo "Error: La ejecución de la consulta falló debido a: \n";
    echo "Query: " . $sql . "\n";
    echo "Errno: " . $mysqli->errno . "\n";
...
$tablausuarios = $resultadu->fetch_assoc();

if ($resultadu->num_rows === 0) {
    echo "Lo sentimos. No se pudo encontrar una coincidencia para el ID $aid. Inténtelo de nuevo.";
    exit;
}
$user=$tablausuarios['nombre']." ".$tablausuarios['apellidos'];	
...
function Header()
{
	...
	//$this->Image('codigo.png',10,8,182);
	...
	$this->Cell(30,10,$this->Image('codigo.png',151,29,38),0);
	$this->Cell(30,10,'','',0,'C');
	$this->Cell(30,10,'','',0,'C');
	$this->Cell(30,10,'','',0,'C');
	$this->Cell(27,10,'','',0,'C');
	$this->Cell(25,12,'CODIGO QR DEL PEDIDO','',0,'C');
	...
	$this->Cell(55,10,"Le Atendio:".$user,1);
	$this->Ln();
...
function FancyTable($header, $data,$data2,$data3,$data4,$data5,$data6)
{
	...
	for($i=0;$i<count($data);$i++){
		...
		$this->Cell($w[4],6,number_format($data3[$i],2),'LR',0,'R',$fill);//celda precio unitario
		$preciopiesas=$data2[$i]*$data3[$i];
		
		$this->Cell($w[5],6,number_format($preciopiesas,2),'LR',0,'R',$fill);//celdas precio total
...
global $anticipo3;
	...
	$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		...
		$this->Cell($w[5],6,"$".number_format($total),'LR',0,'R',$fill);
        $this->Ln();
        $fill = !$fill;
		$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		...
		$this->Cell($w[5],6,"$".number_format($anticipo,2),'LR',0,'R',$fill);
		$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		$this->Ln();
        $fill = !$fill;
...
//*-**--**-ANTICIPO 2 *-*-*-*-*-*-*-*-*-*--*-	
	$this->Cell($w[1],6,"",'LR',0,'L',$fill);
		...
		$this->Cell($w[5],6,"$".number_format($anticipo2,2),'LR',0,'R',$fill);
		$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		$this->Ln();
        $fill = !$fill;
	...
	$this->Cell($w[1],6,"",'LR',0,'L',$fill);
		...
		$this->Cell($w[5],6,"$".number_format($anticipo3,2),'LR',0,'R',$fill);
		$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		$this->Ln();
        $fill = !$fill;
	...
	$xpagar=$total-$anticipo-$anticipo2-$anticipo3;		
		...
		$this->Cell($w[5],6,"$".number_format($xpagar,2),'LR',0,'R',$fill);
		$this->Cell($w[0],6,"",'LR',0,'L',$fill);
		$this->Ln();
        $fill = !$fill;
...
function abajo(){
	...
	*/
	...
	//$this->Cell(30,10,$this->Image('codigo.png',155,30,35),0);
	
	$this->Ln();
	
	//$this->Image('codigo.png',10,8,50);
	//echo 
}
}
...
// Carga de datos
//$pdf->Image("codigo.png",40,10,20,20,"png");
$pdf->SetFont('Arial','',14);
$pdf->AddPage();
$pdf->FancyTable($header,$clavearray,$cantidadarray,$preciovendidoarray,$descripciontapete,$medidastapete,$disenotapete);
$pdf->abajo();

$file='pedidos/'.$aid.'.pdf';
$pdf->Output($file,'I');
$file='pedidosexpo/'.$aid.'.pdf';
$pdf->Output($file,'F');

//$filePDf='/pedidos'.$aid.'PDF';
//$pdf->Output($filePDf,'D');
//****************************************************************************************************************************************************
?>